...
def _source_repository_module() -> Any:
    ...
    conflicting_path = cloud_root / "services" / "multi_step_generation"
...
def _repo_root() -> Path:
    for candidate in Path(__file__).resolve().parents:
        if (candidate / "cloud").is_dir() and (candidate / "pyproject.toml").is_file():
            return candidate
