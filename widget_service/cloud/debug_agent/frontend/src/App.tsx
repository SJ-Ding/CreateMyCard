...
type Health = Record<string, unknown> & { status?: string; model?: string };
...
export default function App() {
  ...
  const processEvent = useCallback((event: DebugEvent) => {
    ...
    if (event.type === 'diagnostic') { addTimeline({ type: event.type, title: textValue(data.kind, '诊断'), detail: textValue(data.message, jsonText(data)), timestamp, tone: 'error' }); return; }
    if (event.type === 'artifact_preview') {
      const index = artifactCounter.current++; setArtifacts((items) => [...items, { ...data, runId: event.runId ?? textValue(data.runId, `run-${items.length + 1}`), timestamp }]); setArtifactIndex(index); addTimeline({ type: event.type, title: 'Artifact 已生成', detail: event.runId ?? '', timestamp, tone: 'success' }); return;
    }
  ...
  const connect = useCallback(() => {
    socketRef.current?.close(); setConnecting(true); const scheme = window.location.protocol === 'https:' ? 'wss' : 'ws'; const socket = new WebSocket(`${scheme}://${window.location.host}/api/v1/ws/main-agent`); socketRef.current = socket;
  ...
  const refreshHealth = useCallback(() => { fetch('/health').then((response) => response.json()).then((value) => setHealth(objectValue(value) as Health)).catch(() => setHealth({ status: 'unavailable' })); }, []);
  useEffect(() => { connect(); refreshHealth(); return () => socketRef.current?.close(); }, [connect, refreshHealth]);
  const send = (payload: Record<string, unknown>) => { if (socketRef.current?.readyState === WebSocket.OPEN) socketRef.current.send(JSON.stringify(payload)); };
  ...
  return <div className="app-shell">
    <header className="topbar"><div className="brand"><div className="brand-mark">{icon('spark')}</div><div><strong>Main Agent</strong><span>Debug Workbench</span></div></div><div className="session-meta"><span className={`status-dot ${connected ? 'online' : 'offline'}`} />{connecting ? '连接中' : connected ? '已连接' : '已断开'}<span className="session-id">会话 {sessionId ? sessionId.slice(0, 8) : '等待会话'}</span></div></header>
